...
이번 장에서는 Chapter 8과 Chapter 9에서 진행한 토큰 집기 예제를 확장하여 **토큰을 쌓는 동작**을 학습합니다. 이번 작업의 목표는 좌측에 있는 빨간 토큰을 먼저 집어 가운데에 놓은 뒤, 우측에 있는 초록 토큰을 집어 빨간 토큰 위에 쌓는 것입니다.

모든 실습은 workspace(`/home/soda/physicai_arm_ws`)에서 진행합니다. 다른 경로에서 진행할 경우, 코드마다 경로를 변경해야 합니다.

## logger_node

`logger_node`는 학습에 필요한 원시 데이터를 에피소드 단위로 저장합니다. 노드를 실행하면 첫 에피소드 기록이 자동으로 시작됩니다. 기록을 마친 뒤 성공이면 `s`, 실패면 `f`를 입력합니다. 다음 에피소드는 `start`를 입력해 시작하고, `q`를 입력하면 종료합니다.

이번 장에서는 저장 폴더를 `dataset_raw_stack`, task name을 `stack`으로 사용합니다.

<details>
<summary>stacking_logger_node 코드</summary>
...
class LoggerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("save_dir", "dataset_raw_stack")
        self.declare_parameter("episode_id", 0)
        self.declare_parameter("task_name", "stack")
...
`parsing_node`는 `logger_node`가 저장한 `dataset_raw_stack` 폴더의 원시 데이터를 읽어 LeRobot이 학습할 수 있는 데이터셋 형식으로 변환합니다. 실패한 에피소드는 제외하고, timestamp를 기준으로 이미지, state, action을 정렬합니다.

이번 장에서는 출력 데이터셋 repo id를 `local/stack_tokens_dataset`으로 사용합니다.

<details>
<summary>stacking_parsing_node 코드</summary>
...
RAW_ROOT = Path("./dataset_raw_stack")
OUT_REPO_ID = "local/stack_tokens_dataset"
...
`inference_node`는 학습된 stacking policy에서 실제 로봇의 센서 입력을 joint target 출력으로 연결합니다. 현재 카메라 이미지와 joint 상태를 입력받아 다음 action을 예측하고, 이를 로봇 제어 명령으로 publish합니다.

이번 장에서는 학습 결과 경로를 `outputs/train/stack_tokens/checkpoints/last/pretrained_model`, 정규화 통계 파일을 `stacking_dataset_stats.json`으로 사용합니다.

<details>
<summary>stacking_inference_node 코드</summary>
...
class StackInferenceNode(Node):
    def __init__(self):
        super().__init__("stacking_inference_node")

        self.declare_parameter("policy_path", "/home/soda/physicai_arm_ws/outputs/train/stack_tokens/checkpoints/last/pretrained_model")
        ...
        stats = load_or_compute_stats("local/stack_tokens_dataset", STATS_CACHE_PATH)
        ...
        self.create_subscription(Image, "/arm/top_cam", self._on_top_image, qos_profile_sensor_data)
        self.create_subscription(Image, "/arm/front_cam", self._on_front_image, qos_profile_sensor_data)
        ...
        self.get_logger().info(f"StackInferenceNode started. fps={self.fps} device={self.device}")
...
```python
...
            "stacking_logger_node = physicai_arm.stacking_logger_node:main",
            "stacking_parsing_node = physicai_arm.stacking_parsing_node:main",
            "stacking_inference_node = physicai_arm.stacking_inference_node:main",
...
```
