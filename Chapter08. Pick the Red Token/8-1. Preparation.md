...
로그로 저장될 데이터들의 topic을 구독합니다. `/arm/front_cam`, `/arm/top_cam`, `/follower/joint_states`, `/follower/joint_targets` 등의 정보를 받습니다. 각 topic에 데이터가 들어오면 콜백 함수가 실행됩니다. 콜백 함수는 데이터를 바로 파일에 저장하지 않고 가장 최근 데이터를 임시로 보관하는 역할입니다.
