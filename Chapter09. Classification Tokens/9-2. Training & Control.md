...
- `local/classify_tokens_dataset` 데이터셋으로 저장합니다.
...
데이터가 준비되면 LeRobot을 활용하여 ACT policy를 학습합니다. 이번 예제의 입력 데이터 위치와 출력 위치는 Chapter8과 다르게 `classify_tokens` 기준으로 설정합니다.

- 배치 사이즈(batch_size): 8
- 스텝 수(steps): 100000
- 입력 데이터 위치(dataset.repo_id): local/classify_tokens_dataset
- 출력 데이터 위치(output_dir): outputs/train/classify_tokens
- 작업 이름(job_name): classify
...
lerobot-train \
  --policy.type=act \
  --dataset.repo_id=local/classify_tokens_dataset \
  --batch_size=8 \
  --steps=100000 \
  --output_dir=outputs/train/classify_tokens \
  --job_name=classify \
  --policy.device=cuda \
  --wandb.enable=false \
...
```sh
lerobot-train \
  --config_path=outputs/train/classify_tokens/checkpoints/last/pretrained_model/train_config.json \
  --resume=true
```
...
학습된 checkpoint/model이 준비되었다면 학습된 모델을 이용하여 로봇을 제어합니다. 로봇이 실행 중 과하게 움직이거나 멈춰야 할 때에는 `classification_inference_node`를 실행 중인 터미널에서 `Ctrl+C`를 눌러 노드 실행을 중단할 수 있습니다.
...
2. `classification_inference_node`가 `/arm/top_cam`, `/arm/front_cam`, `/follower/joint_states`를 subscribe합니다.
