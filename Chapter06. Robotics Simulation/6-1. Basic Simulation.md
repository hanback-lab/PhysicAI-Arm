...
Gazebo Fortress는 공식적으로 URDF를 지원합니다. 내부적으로 libsdformat 라이브러리가 URDF를 SDF로 자동 변환해서 로드하는 방식입니다. 일반적으로는 create service나 spawn 도구를 이용해 삽입합니다. Gazebo를 연 뒤, 물체를 스폰하는 명령어는 아래와 같습니다.
...
URDF는 단일 로봇 모델만 기술하는 포맷이므로 SDF의 `<world>`, `<light>` 같은 일부 요소는 직접 변환되지 않습니다. 바닥면이나 조명은 SDF world에서 별도로 정의하거나 Gazebo 기본 world를 활용해야 합니다.
...
환경 설정 충돌을 방지하기 위해 가상환경을 사용하여 설치하겠습니다.
...
해당 가상환경은 추후 모방학습 후 추론 단계에서도 사용됩니다.

`/home/<사용자명>/lerobot_infer_env/bin/python` 라는 메시지가 나오면 성공입니다. 다르게 뜨는 경우 위의 source 명령을 한 번 더 실행해주세요. 가상환경이 설정되었으면 MuJoCo를 설치하겠습니다.
